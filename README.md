...
The program is written in [Racket](https://racket-lang.org) and comes with a simple `Makefile`:

```sh
$ make             # install
$ make doc         # documentation (if you want)
$ make uninstall   # uninstall
```

The program and the documentation (this file) are put in `~/.local/bin`: just make sure the location you want occurs in `$PATH`. If you want to change that, edit the path in this line of `Makefile`:
